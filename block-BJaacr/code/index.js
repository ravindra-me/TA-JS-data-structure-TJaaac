...
for (const house of got.houses[0].people) {
  console.log(house.name);
}
// 5. Log the name of all the people in house Lannisters (index 1) one by one
for (const house of got.houses[1].people) {
  console.log(house.name);
}
...
for (const house of got.houses) {
  for (const detail of house.people) {
...
for (const house of got.houses) {
  for (const detail of house.people) {
    allDescription.push( detail.description);
  }
}
